...
async function main() {
  ...
  const aliceAvatar = await seedAvatar(join(import.meta.dirname, './assets/alice.jpg'))
  const bobAvatar = await seedAvatar(join(import.meta.dirname, './assets/bob.webp'))

  await db.update(user).set({ image: aliceAvatar }).where(eq(user.email, alice.email))
  await db.update(user).set({ image: bobAvatar }).where(eq(user.email, bob.email))
